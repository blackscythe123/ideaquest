...
// Activity (awake/asleep) detection constants
const EYE_EAR_THRESHOLD = 0.30; // Higher threshold detects closures sooner
const SLEEP_MS = 800; // quicker to mark slept
const WAKE_MS = 250; // quicker to mark active
const GONE_MS = 3000; // Time with no face detected before marking as 'gone'

// Utility functions for activity detection
...
function computeEAR(landmarks, eye) {
  // eye is array of 6 indices: [p1, p2, p3, p4, p5, p6]
...
function computeNormalizedEAR(landmarks, leftEye, rightEye) {
  // Calculate individual eye EARs
  const leftEAR = computeEAR(landmarks, leftEye);
  const rightEAR = computeEAR(landmarks, rightEye);
  const avgEAR = (leftEAR + rightEAR) / 2;
  
  // Calculate face size for normalization using key facial landmarks
  ...
  // Face dimensions for size estimation
  const faceHeight = euclideanDistance(noseTip, chin);
  const faceWidth = euclideanDistance(leftFace, rightFace);
  const faceSize = Math.sqrt(faceHeight * faceWidth); // Geometric mean of dimensions
  
  // Normalize EAR by face size - larger faces get higher threshold tolerance
  const sizeNormalizationFactor = Math.max(0.5, Math.min(2.0, faceSize / 0.15));
  const normalizedEAR = avgEAR / sizeNormalizationFactor;
  
  return {
    rawEAR: avgEAR,
    normalizedEAR: normalizedEAR,
    faceSize: faceSize,
    normalizationFactor: sizeNormalizationFactor
  };
}
...
class Participant {
  ...
  createTile() {
    ...
    activityElement.className = 'activity-status unknown';
    ...
    if (currentVideoQuality !== 'AUDIO_ONLY' || this.isLocal) {
      ...
      if (stream && stream.getVideoTracks().length > 0) {
        ...
        if (shouldShowVideo) {
          ...
          // Start activity detection when we have video
          this.initActivityDetection();
        } else {
          this.videoElement.style.display = 'none';
          noVideo.style.display = 'flex';
          // Stop activity detection when no video
  ...
  initActivityDetection() {
    // Only one FaceMesh per participant
    if (this.faceMesh || !this.videoElement) return;
    // Skip for remote tiles without visible video element yet
    try {
      ...
      fm.setOptions({
        ...
        minTrackingConfidence: 0.3,   // Better tracking continuity
        staticImageMode: false,       // Enable tracking mode for video
        enableFaceGeometry: false     // Disable heavy geometry for better performance
      });

      this.faceMesh = fm;
      this.lastDetectionState = 'no-face'; // 'face-open', 'face-closed', 'no-face'
      this.stateSince = performance.now();

      fm.onResults((results) => {
        let detectionState = 'no-face'; // 'face-open', 'face-closed', 'no-face'
        ...
        if (results.multiFaceLandmarks && results.multiFaceLandmarks.length > 0) {
          ...
          // Use distance-normalized EAR calculation
          const earData = computeNormalizedEAR(lm, LEFT_EYE, RIGHT_EYE);
          const { rawEAR, normalizedEAR, faceSize, normalizationFactor } = earData;
          
          // Use normalized EAR for more consistent detection across distances
          detectionState = normalizedEAR < EYE_EAR_THRESHOLD ? 'face-closed' : 'face-open';
          
          // Debug logging with distance information
          if (this.isLocal && Math.random() < 0.1) { // Log 10% of frames to avoid spam
            console.log(`👁️ EAR Debug - Raw: ${rawEAR.toFixed(3)}, Normalized: ${normalizedEAR.toFixed(3)}, Face Size: ${faceSize.toFixed(3)}, Factor: ${normalizationFactor.toFixed(2)}`);
          }
        } else {
          // No face observed
        ...
        if (detectionState !== this.lastDetectionState) {
          this.lastDetectionState = detectionState;
          this.stateSince = now;
        }

        // Decide status with dwell time
        let status = 'unknown';
        const timeSinceChange = now - this.stateSince;
        
        if (this.lastDetectionState === 'face-open' && timeSinceChange > WAKE_MS) {
          status = 'active';
        } else if (this.lastDetectionState === 'face-closed' && timeSinceChange > SLEEP_MS) {
          status = 'slept';
        } else if (this.lastDetectionState === 'no-face' && timeSinceChange > GONE_MS) {
          status = 'gone';
        }
        
        // Debug log for local participant activity detection
        if (this.isLocal && status !== 'unknown') {
          const earData = results.multiFaceLandmarks?.[0] ? 
            computeNormalizedEAR(results.multiFaceLandmarks[0], [33, 160, 158, 133, 153, 144], [362, 385, 387, 263, 373, 380]) : 
            null;
          const earInfo = earData ? 
            `Raw EAR: ${earData.rawEAR.toFixed(3)}, Normalized: ${earData.normalizedEAR.toFixed(3)}, Size: ${earData.faceSize.toFixed(3)}` : 
            'N/A';
          console.log(`👁️ Local activity detected: ${status} (State: ${this.lastDetectionState}, ${earInfo})`);
        }
        
        this.updateActivityStatus(status);
      });
      ...
      const processFrame = async () => {
        ...
        // Ensure dimensions are ready; if not, try again soon
      ...
      // Start once the video is actually playing for faster readiness
      const startLoop = () => {
      ...
      if (this.videoElement.readyState >= 2) {
        ...
        this.videoElement.addEventListener('playing', startLoop, { once: true });
        this.videoElement.addEventListener('loadeddata', startLoop, { once: true });
      }

      // Avoid indefinite checking: assume active after brief warmup if no signal yet
      this.updateActivityStatus('unknown');
      setTimeout(() => {
        if (this && this.activityElement && (!this.lastDetectionState || this.lastDetectionState === 'no-face')) {
          // If still no detection after warmup, assume person is present and active
  ...
  updateActivityStatus(status) {
    ...
    // Update local UI
    this.activityElement.classList.remove('active', 'slept', 'unknown', 'gone');
    switch (status) {
      case 'active':
        this.activityElement.classList.add('active');
        this.activityElement.textContent = '🙂 Active';
        break;
      case 'slept':
        this.activityElement.classList.add('slept');
        this.activityElement.textContent = '😴 Slept';
        break;
      case 'gone':
        this.activityElement.classList.add('gone');
        this.activityElement.textContent = '👻 Gone';
        break;
      default:
        this.activityElement.classList.add('unknown');
        this.activityElement.textContent = '⏳ Checking...';
    }
    
    // If this is the local participant, broadcast the status to all peers
    if (this.isLocal) {
      this.broadcastActivityStatus(status);
    }
  }
  
  broadcastActivityStatus(status) {
    // Send activity status to all connected peers via data channels
    peerConnections.forEach((pc, peerId) => {
      if (pc.activityChannel && pc.activityChannel.readyState === 'open') {
        try {
          ...
          console.log(`👁️ Broadcasted activity status '${status}' to ${peerId}`);
        } catch (error) {
          console.error(`Error broadcasting activity status to ${peerId}:`, error);
        }
      }
    });
  }
  
  updateActivityStatusFromRemote(status) {
    // This method is called when we receive activity status from a remote participant
    // It only updates the UI, doesn't trigger further broadcasts
    if (!this.activityElement) return;
    ...
    switch (status) {
      case 'active':
        this.activityElement.classList.add('active');
        // Add indicator that this status is transmitted, not locally detected
        this.activityElement.textContent = currentVideoQuality === 'AUDIO_ONLY' ? '🙂 Active (Audio)' : '🙂 Active';
        break;
      case 'slept':
        this.activityElement.classList.add('slept');
        this.activityElement.textContent = currentVideoQuality === 'AUDIO_ONLY' ? '😴 Slept (Audio)' : '😴 Slept';
        break;
      case 'gone':
        this.activityElement.classList.add('gone');
        this.activityElement.textContent = currentVideoQuality === 'AUDIO_ONLY' ? '👻 Gone (Audio)' : '👻 Gone';
        break;
      default:
        this.activityElement.classList.add('unknown');
        this.activityElement.textContent = currentVideoQuality === 'AUDIO_ONLY' ? '⏳ Waiting...' : '⏳ Checking...';
    }
  }
  ...
  participants.forEach((participant, participantId) => {
    ...
    if (participantId === 'local') {
      ...
      if (!isCameraOff && localStream && localStream.getVideoTracks().length > 0) {
        ...
        // Start activity detection for local user since video is visible
        participant.initActivityDetection();
      ...
      if (isAudioOnly) {
        ...
        // Stop local activity detection for remote participants since video is hidden
        // But keep the activity status element visible - it will be updated via data channel
        participant.stopActivityDetection();
        
        // Set status to show we're waiting for transmitted data
        if (participant.activityElement) {
          participant.updateActivityStatusFromRemote('unknown');
        }
        ...
        if (participant.stream && participant.stream.getVideoTracks().length > 0) {
          ...
          // Restart activity detection for remote participants when video is restored
          participant.initActivityDetection();
        } else {
          videoElement.style.display = 'none';
          noVideoElement.style.display = 'flex';
...
function createPeerConnection(participantId) {
  ...
  // Create activity status data channel for transmitting awake/sleep status
  const activityChannel = pc.createDataChannel('activity', { ordered: true });
  activityChannel.onopen = () => {
    console.log(`👁️ Activity status channel opened with ${participantId}`);
  };
  
  // Store activity channel reference for later use
  pc.activityChannel = activityChannel;
  ...
  pc.ondatachannel = (event) => {
    ...
    } else if (channel.label === 'activity') {
      console.log(`👁️ Received activity channel from ${participantId}`);
      channel.onmessage = (event) => {
        const data = JSON.parse(event.data);
        if (data.type === 'activity-status') {
          // Update remote participant's activity status
          const participant = participants.get(participantId);
          if (participant) {
            console.log(`👁️ Received activity status from ${participantId}: ${data.status}`);
            participant.updateActivityStatusFromRemote(data.status);
          }
        }
      };
      
      // Store reference to activity channel for this peer
      const pc = peerConnections.get(participantId);
      if (pc) {
        pc.activityChannel = channel;
      }
    }
  };
