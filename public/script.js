    ...
    try {
      ...
      fm.onResults((results) => {
        ...
        if (results.multiFaceLandmarks && results.multiFaceLandmarks.length > 0) {
          ...
          detectionState = ear < EYE_EAR_THRESHOLD ? 'face-closed' : 'face-open';
        } else {
          // No face observed
        ...
        if (this.isLocal && status !== 'unknown') {
          const earValue = results.multiFaceLandmarks?.[0] ? 
            ((computeEAR(results.multiFaceLandmarks[0], [33, 160, 158, 133, 153, 144]) + 
              computeEAR(results.multiFaceLandmarks[0], [362, 385, 387, 263, 373, 380])) / 2).toFixed(3) : 'N/A';
          console.log(`👁️ Local activity detected: ${status} (State: ${this.lastDetectionState}, EAR: ${earValue})`);
        }
